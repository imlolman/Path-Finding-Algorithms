...
var inputMode = "wall";
var showExplored = true;

function resetHW(){
    w = window.innerWidth;
    h = canvasBox.clientHeight;
}
function setup(){
    resetHW()
    if(h/w > 1){
        document.getElementsByClassName("overlay1")[0].style.display = "none"
        ...
        grids = new Grids(w, h)
        if(window.mobileCheck() && !document.fullscreen){
            document.getElementsByClassName("overlay1")[0].style.display = "flex"
        }else{
            document.getElementsByClassName("overlay1")[0].style.display = "none"
        }
        loop()
...
function navBtnFunction(action){
    inputMode = action
    document.getElementById("begin").classList.remove("active")
    document.getElementById("blank").classList.remove("active")
...
function toggleShowExplored(){
    showExplored = !showExplored
    if(showExplored){
        document.getElementById("showExplored").classList.add("active")
    }else{
        document.getElementById("showExplored").classList.remove("active")
    }
}
